        ...
        <table class="w3-table-all">
            ...
            @foreach ($books as $book)
                <tr>
                    ...
                    <td class="w3-center"><a href="{{ route('books.show', $book->id) }}">{{ $book->title }}</a>
                    </td>
                    <td class="w3-center">{{ $book->isbn }}</td>
                    <td class="w3-center">{{ Str::limit($book->desc, 40, '...') }}</td>
                    ...
                    <td>
                        <button type="submit" class="btn btn-danger" form="del-book" onclick="return confirm('هل أنت متأكد ؟')"><i
                                class="fa-solid fa-trash"></i></button>
                    </td>
                </tr>
                <form method="POST" action="{{ route('books.destroy', $book->id) }}" id="del-book">@csrf
                    @method('DELETE')
                </form>
            @endforeach
        </table>
