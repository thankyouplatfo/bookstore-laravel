...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header"><h1>{{ __('site.glossary_of_classification_books') }}</h1></div>
                                        ...
                                        <div class="col-lg-4 col-6 mb-3">
                                            <div class="d-block md-2 h-100 border rounded p-1">
                                                ...
                                                @if ($book->category != null)
                                                    <a href="{{ route('categories.show',$book->category) }}" style="color:grey">{{ $book->category->name }}</a><br>
                                                @endif
                                                @if ($book->authors->isNotEmpty())
                                                    <b>{{ __('site.composing') }}</b>
                                                    @foreach ($book->authors as $author)
                                                        {{ $loop->first ? ' ' : 'و' }}
                                                        <a href="{{ route('authors.show',$author) }}" style="color: grey">{{ $author->name }}</a> <br>
                                                    @endforeach
                                                @endif
